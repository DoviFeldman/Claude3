...
// ---------- DeepSeek v4 via NVIDIA (implemented, default) ----------
// Free NVIDIA API, OpenAI-compatible. Thinking is turned OFF.
// Key is read from ANTHROPIC_API_KEY (kept that env var name) or NVIDIA_API_KEY.
async function streamDeepSeek({ systemPrompt, userPrompt, onText }) {
  ...
  const stream = await client.chat.completions.create({
    ...
    top_p: 0.95,
    max_tokens: 16384,
    // disable DeepSeek's thinking mode (Node SDK: extra body fields go inline)
