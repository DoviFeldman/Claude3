...
// Default model is qwen/qwen3.6-27b — a reasoning model. We DISABLE reasoning
// (reasoning_effort:"none") because otherwise it can spend the whole token
// budget "thinking" and return empty content for a structured task; we also
// hide any reasoning and strip stray markdown code fences, so only the clean
// protocol/HTML reaches the browser. Free tier is ~8000 tokens/min.
async function streamGroq({ systemPrompt, userPrompt, onText, model: modelOverride }) {
  ...
  const params = {
    ...
    temperature: 1,
    max_tokens: 4096,
    reasoning_format: "hidden", // strip any reasoning from the returned content
    stream: true,
  };
  // Qwen reasoning models accept reasoning_effort "none" | "default"; "none"
  // turns thinking off entirely (fast, and no wasted token budget). Other
  // providers/models may not support this flag, so only send it for Qwen.
